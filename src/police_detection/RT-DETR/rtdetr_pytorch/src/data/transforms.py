...
# Fallback if torchvision.datapoints is not available
class Image(torch.Tensor):
    pass

class BoundingBox(torch.Tensor):
    def __new__(cls, data, format=None, spatial_size=None):
        obj = torch.Tensor.__new__(cls, data)
        obj.format = format
        obj.spatial_size = spatial_size
        return obj

# Create dummy enum-like class for BoundingBoxFormat
...
@register
class ToImageTensor(T.ToTensor):
    def __init__(self):
        super().__init__()
...
class PadToSize(T.Pad):
    _transformed_types = (
        Image,         # your fallback Image class
        BoundingBox,   # your fallback BoundingBox class
        torch.Tensor,  # optionally support tensor input
    )
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:
        fill = self._fill.get(type(inpt), 0)
...
class ConvertBox(T.Transform):
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:  
        if self.out_fmt:
            ...
            inpt = BoundingBox(inpt, format=self.data_fmt[self.out_fmt], spatial_size=spatial_size)
