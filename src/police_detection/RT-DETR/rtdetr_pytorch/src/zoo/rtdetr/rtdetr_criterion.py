...
# from torchvision.models.detection._utils import nested_tensor_from_tensor_list
...
def nested_tensor_from_tensor_list(tensor_list):
    """
    Create a NestedTensor from a list of tensors with shape [C, H, W].
    Pads all to the maximum height and width in the list.
    """
    if tensor_list[0].ndim != 3:
        raise ValueError("Expected 3D tensors (C, H, W)")
    ...
    padded_tensors = torch.zeros((batch_size, max_c, max_h, max_w), dtype=dtype, device=device)
    mask = torch.ones((batch_size, max_h, max_w), dtype=torch.bool, device=device)

    for i, img in enumerate(tensor_list):
        c, h, w = img.shape
        padded_tensors[i, :c, :h, :w] = img
        mask[i, :h, :w] = False  # False where data is present
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses, alpha=0.2, gamma=2.0, eos_coef=1e-4, num_classes=5):
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_score_o = target_score_o.clone()
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # Convert to xyxy and fix invalid boxes
        src_boxes_xyxy = box_cxcywh_to_xyxy(src_boxes)
        target_boxes_xyxy = box_cxcywh_to_xyxy(target_boxes)

        # Ensure x2 >= x1 and y2 >= y1 using out-of-place ops
        x1y1 = src_boxes_xyxy[..., :2]
        x2y2 = src_boxes_xyxy[..., 2:]
        x2y2_clamped = torch.max(x2y2, x1y1 + 1e-6)
        src_boxes_xyxy = torch.cat([x1y1, x2y2_clamped], dim=-1)

        x1y1_t = target_boxes_xyxy[..., :2]
        x2y2_t = target_boxes_xyxy[..., 2:]
        x2y2_clamped_t = torch.max(x2y2_t, x1y1_t + 1e-6)
        target_boxes_xyxy = torch.cat([x1y1_t, x2y2_clamped_t], dim=-1)
...
def dice_loss(inputs, targets, num_boxes):
    ...
    Args:
        inputs: predictions (N, H*W)
        targets: targets (N, H*W)
        num_boxes: normalization factor
    Returns:
        loss: scalar
    """
